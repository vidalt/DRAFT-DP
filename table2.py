...
for dataset in datasets:
    with open(f'experiments_results/Results_main_paper_N_unknown/N_free_{dataset}_results.json', 'r') as f:
        results = json.load(f)
        ...
        for n_trees in tree_values:
            for epsilon in epsilon_values:
                ...
                if error and len(error) != 5:
                    #assert(len(error) == 5)
                    print("Missing runs for", dataset, depth, n_trees, epsilon, "found", len(error))
                    if len(error) > 5:
                        print(error)
